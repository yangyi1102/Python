    ...
    def Get_first_Page(self,url):                                                    #第一页获取
        ...
        while True:
            try:
                ...
                proxyUser = "HEH76239M169533D"
                proxyPass = "94204281631707F8"
                ...
                response=requests.get(url,headers=self.header,proxies=proxies)     #获得第一页
                return response.text
            except ProxyError  as e:                      #代理报错
                count=count+1
                time.sleep(2)                             
                if count>=5:                                    #达到5次
                    print('第79行',e)
                    t=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")                                 #获得时间
                    with open("pagelink.txt", "a",encoding='utf-8') as f:                                                   #打开链接
                        f.write(t)
                        f.write("\n" )
                        f.write(url)
                        f.write("\n" )
    ...
    def Get_page(self,url):
        ...
        while True:                                              
            try:
                ...
                session = requests.Session()                                                     #cookie2获取
                ...
                if count>=5:
                    ...
                    with open("pagelink.txt", "a",encoding='utf-8') as f:
                        ...
                        f.write("\n" )
                    return False    
    def Get_Page_list(self,response,url):                                           #获取页面商品链接  
        li=[]
        try:
            ...
            with open("pagelink.txt", "a",encoding='utf-8') as f:
                ...
                f.write("\n" )
            return False
    def GetResponse(self,url):                           #商品详情获取
        count=0
        while True:
            try:
                response=requests.get(url,headers=self.header)               #获得详情目前不需要代理
                return response.text
            except Exception as e:
                print(e)
                count=count+1
                ...
                if count>=5:
                    with open("1.txt", "a",encoding='utf-8') as f:
                        ...
                        f.write("\n" )
                    return False                                             #错误
    def Get_Goods_inf(self,response,url):                  #商品信息解析
        dict=dic
        try:
...
class SpiderMan(object):                        
    def __init__(self):
        self.aibb=Aibb()                      
        self.q=Queue()
    def Run_first_Page(self):                                                                 # 获取第一页
        lis=self.aibb.Get_search_Page()                                                    #  获得搜索页
        if lis is not False:
            for i in lis:
    ...
    def Run(self,count):
        for i in url_config:
            url=i.format(count=count)                                   #商品列表链接获得商品的链接
            response=self.aibb.Get_page(url)
            lis=self.aibb.Get_Page_list(response, url)             #商品详情链接列表
            if lis is not False:                                                     #列表不为空
                while len(lis) is not 0:                                         #获得
                    i=lis.pop()
                    text=self.aibb.GetResponse(i)
                    if text is not False:
                        inf=self.aibb.Get_Goods_inf(text,i)
                        self.q.put(inf)
    def Save_Inf(self):                                                                  #数据库保存信息
        ...
        cur=db.cursor()
        while True:
            dict=self.q.get()                                                     #从队列获得一个
            print(dict)
            if dict['shopurl'] is '':                                              #检测信息是否完整
...
def Main():
    spiderman=SpiderMan()
    tpool=[]                                         #线程列表
    tpool.append(threading.Thread(target=spiderman.Run_first_Page))         #  加入第一页
    tpool.append(threading.Thread(target=spiderman.Save_Inf))                    #加入mysql'线程
    count=2                                                                                                              #第几页
    for i in range(98):                                                                                              #页数
